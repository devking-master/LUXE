         ...
         <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
               <h1 className="text-lg md:text-xl font-bold tracking-tight text-zinc-900 flex items-center gap-2">
                  <span className="truncate max-w-[150px] sm:max-w-none">Order #{order._id.slice(-6).toUpperCase()}</span>
               ...
               <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden shadow-sm">
                  ...
                  <div className="divide-y divide-zinc-100">
                     {order.products?.map((p) => (
                        <div key={p._id} className="p-3 md:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 hover:bg-zinc-50 transition-colors">
                           <div>
                              <p className="text-sm md:text-base font-medium text-zinc-900">{p.title}</p>
                              <p className="text-xs text-zinc-500 mt-0.5">Qty: {p.quantity}</p>
                           </div>
                           <p className="text-sm md:text-base font-medium text-zinc-900 self-start sm:self-auto">₦{(p.price * p.quantity).toLocaleString()}</p>
                        </div>
                     ))}
                  </div>
            ...
            <div className="space-y-4 md:space-y-6">
               <div className="bg-white border border-zinc-200 rounded-xl p-4 md:p-6 shadow-sm">
                  ...
                  <div className="space-y-3 md:space-y-4">
                     <div>
                        <p className="text-[10px] md:text-xs text-zinc-500 uppercase tracking-wide mb-1">Name</p>
                        <p className="text-sm md:text-base font-medium text-zinc-900">{order.customerName || "Guest User"}</p>
                     </div>
                     <div>
                        <p className="text-[10px] md:text-xs text-zinc-500 uppercase tracking-wide mb-1">ID</p>
               ...
               <div className="bg-white border border-zinc-200 rounded-xl p-4 md:p-6 shadow-sm">
                  ...
                  <div className="space-y-3 md:space-y-4">
                     <div>
                        <p className="text-[10px] md:text-xs text-zinc-500 uppercase tracking-wide mb-1">Date Placed</p>
                     ...
                     <div>
                        <p className="text-[10px] md:text-xs text-zinc-500 uppercase tracking-wide mb-1">Payment Status</p>
                        <span className="inline-flex items-center gap-1.5 px-2 py-1 bg-emerald-50 text-emerald-700 rounded-md text-[10px] md:text-xs font-medium mt-1">
